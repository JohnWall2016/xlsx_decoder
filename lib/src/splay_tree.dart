 ...
 * `fromIterable`.
 */
void _fillMapWithMappedIterable(
 ...
 * `fromIterables`.
 */
void _fillMapWithIterables(Map map, Iterable keys, Iterable values) {
...
class SplayTreeMap<K, V> extends _SplayTree<K, _SplayTreeMapNode<K, V>>
  ...
  factory SplayTreeMap.fromIterable(Iterable iterable,
    ...
    _fillMapWithMappedIterable(map, iterable, key, value);
    return map;
  }
  ...
  factory SplayTreeMap.fromIterables(Iterable<K> keys, Iterable<V> values,
      [int compare(K key1, K key2), bool isValidKey(potentialKey)]) {
    SplayTreeMap<K, V> map = new SplayTreeMap<K, V>(compare, isValidKey);
    _fillMapWithIterables(map, keys, values);
    return map;
  }
  ...
  void nodesFrom(K key, f(_SplayTreeMapNode<K, V> node)) {
    Iterator<_SplayTreeNode<K>> nodes =
        new _SplayTreeNodeIterator<K>.startAt(this, key);
    while (nodes.moveNext()) {
      _SplayTreeMapNode<K, V> node = nodes.current;
      f(node);
    }
